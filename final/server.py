...
def create(call):
		...
		if 'username' in session and session['username'] == username:
			...
			for user in response:
				...
				if distance.distance(r_position,u_position).miles <= u_distance:
					data.append(user[1]);
			for user in data:
				pusher.trigger('private-'+user,'new_room',{'room':name,'username':username})
	...
	if call == "user":
		...
		if data:
			...
			password = request.form['password']
			query= "INSERT INTO users VALUES (default,'"+username+"','"+password+"',default,"+request.form['lat']+","+request.form['long']+");"
			cursor = mysql.connection.cursor()
			cursor.execute(query)
			mysql.connection.commit();
			session['username'] = username
	...
	if call == "spoof":
		...
		if 'username' in session and session['username'] == username:
			...
			delta = request.form['time']
			time = datetime.now() + timedelta(hours=int(delta))
			query= "INSERT INTO rooms VALUES (default,'"+name+"','"+str(time)+"','"+username+"',"+request.form['lat']+","+request.form['long']+");"
			cursor = mysql.connection.cursor()
			cursor.execute(query)
			mysql.connection.commit();
			query = "SELECT * FROM users;"
			...
			response = cursor.fetchall()
			data = list()
			r_position = (request.form['lat'],request.form['long'])
			for user in response:
				u_position = (user[4],user[5])
				u_distance = user[3]
				if distance.distance(r_position,u_position).miles <= u_distance:
...
def get(call,room=None):
	...
	if 'username' in session and session['username'] == username:
		cursor = mysql.connection.cursor()
		query = "DELETE FROM rooms WHERE expire < CURRENT_TIMESTAMP();"
		cursor.execute(query)
		mysql.connection.commit();
		if call == "messages":
			cursor = mysql.connection.cursor()
